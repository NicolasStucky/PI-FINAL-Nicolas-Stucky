...
const Form = () => {
  ...
  const validate = () => {
    ...
    if(formData.dificultad < 1 && formData.dificultad > 5){
      newErrors.dificultad = "Elija el nivel de dificultad del 1 al 5"
    }
    ...
    if (formData.name.length < 1 && formData.name === "") {
  ...
  const handleChange = (event) => {
    ...
    if (name === "countries") {
      const selectedCountryIds = Array.from(options).filter((option) => option.selected).map((option) => option.value);
